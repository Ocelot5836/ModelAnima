...
public class TestLayer extends LayerRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>>
{
    ...
    public void render(MatrixStack matrixStack, IRenderTypeBuffer buffer, int packedLight, AbstractClientPlayerEntity player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        ...
        if (geometryModel instanceof AnimatedModel)
        {
            ...
            ((AnimatedModel) geometryModel).applyAnimation(ticks / 20F, animation, MolangRuntime.runtime().setQuery("target_x_rotation", player.getViewXRot(partialTicks)).setQuery("target_y_rotation", player.getViewYRot(partialTicks)).setQuery("model_scale", 1.0F).setQuery("modified_distance_moved", player.moveDist).setQuery("modified_move_speed", player.getSpeed()).setQuery("is_baby", player.isBaby() ? 1.0F : 0.0F).setQuery("on_fire_time", (float)player.getRemainingFireTicks() / 20.0F).setQuery("is_first_person", player == Minecraft.getInstance().getCameraEntity() && Minecraft.getInstance().options.getCameraType() == PointOfView.FIRST_PERSON ? 1.0F : 0.0F));
        }
